(function (factory) {
  ...
  var DEFAULT_PENALITIES = {
    HANGING_PUNCTUATION: {
      illegal: function illegal(_ref) {
        var lines = _ref.lines;
        return lines.find(function (line) {
          return /^\W/.test(line.value);
        });
      }
    },
    TRUNCATIONS: {
      illegal: function illegal(_ref2) {
        var words = _ref2.words,
            lines = _ref2.lines;
        var truncatedWords = words.filter(function (word) {
          return !lines.find(function (line) {
            return line.value.includes(word);
          });
        });
    ...
    WIDOWS: {
      ...
      ignore: function ignore(_ref3) {
        var lines = _ref3.lines,
            words = _ref3.words;
        var hasWidow = lines[lines.length - 1] === words[words.length - 1];
        return !hasWidow;
      }
    },
    EOL_SHORT_WORDS: {
      weight: 0.5,
      ignore: function ignore(_ref4) {
        var lines = _ref4.lines;
        return lines.length <= 2;
      },
      value: function value(_ref5) {
        var lines = _ref5.lines;
        var linesWithShortWordsAtEnd = lines.filter(function (line) {
          return /\b\w{1,3}\s?$/.test(line.value);
        });
    ...
    UNBALANCED_LINES: {
      ...
      value: function value(_ref7) {
        ...
        var normalizedWidths = lines.map(function (line) {
          return line.width / maxWidth;
        });
        var mean = normalizedWidths.reduce(function (sum, len) {
          return sum + len;
        }, 0) / lines.length; // SEE https://en.wikipedia.org/wiki/Standard_deviation

        var deviation = Math.sqrt(1 / lines.length * normalizedWidths.reduce(function (sum, len) {
          return sum + Math.pow(len - mean, 2);
        }), 0);
        var coefVariation = deviation / mean;
        return 1 - coefVariation;
      }
    }
  };
  function Drapeau(text, _temp) {
    var _ref8 = _temp === void 0 ? {} : _temp,
        _ref8$step = _ref8.step,
        step = _ref8$step === void 0 ? 1 : _ref8$step,
        _ref8$minFontSize = _ref8.minFontSize,
        minFontSize = _ref8$minFontSize === void 0 ? 12 : _ref8$minFontSize,
        _ref8$maxFontSize = _ref8.maxFontSize,
        maxFontSize = _ref8$maxFontSize === void 0 ? 72 : _ref8$maxFontSize,
        _ref8$width = _ref8.width,
        width = _ref8$width === void 0 ? 300 : _ref8$width,
        _ref8$measure = _ref8.measure,
        measure = _ref8$measure === void 0 ? function (glyph, fontSize) {
      return {
        width: 1,
        height: 1
      };
    } : _ref8$measure,
        _ref8$penalties = _ref8.penalties,
        penalties = _ref8$penalties === void 0 ? function (clonedDefaultPenalities) {
      return clonedDefaultPenalities;
    } : _ref8$penalties;

    penalties = penalties(lodash_clonedeep(DEFAULT_PENALITIES));
    var candidates = []; // Create an array of words which will be exposed to the context of penalties

    var words = text.split(/\s/);

    for (var fontSize = minFontSize; fontSize < maxFontSize; fontSize += step) {
      var lines = wordWrapper.lines(text, {
        width: width,
        mode: 'wrap',
      ...
      for (var _i = 0, _Object$values = Object.values(penalties); _i < _Object$values.length; _i++) {
        var penalty = _Object$values[_i];
        if (penalty.weight <= 0) continue;
        if (penalty.ignore && penalty.ignore(context)) continue;

        if (penalty.illegal && penalty.illegal(context)) {
          score = 0;
          break;
        }
    ...
    function createCachableMeasure(fontSize) {
      return function (text, start, end, lineWidth) {
        ...
        for (var index = start; index < end; index++) {
          var glyph = text.charAt(index);

          var _char = measure(glyph, fontSize);

          totalWidth += _char.width;
          if (_char.height > maxHeight) maxHeight = _char.height;
  ...
  function render(width, height) {
    ...
    var result = Drapeau(text, {
      step: 1,
      minFontSize: 30,
      maxFontSize: 50,
      width: canvas.width / dpi,
      measure: function measure(glyph, fontSize) {
      ...
      penalties: function penalties(_penalties) {
        _penalties.HEIGHT_DIFFERENCE = {
          weight: 0.1,
          value: function value(_ref) {
            var lines = _ref.lines;
            var height = Math.max.apply(Math, lines.map(function (line) {
              return line.height;
            }));
            var deltaHeight = (canvas.height - height) / canvas.height;
    ...
    console.log(result);
    ctx.scale(dpi, dpi); // Render all candidates
    ...
    result.candidates.forEach(function (candidate, index) {
      ...
      candidate.lines.forEach(function (line, index) {
